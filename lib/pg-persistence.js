...
// Commented out old SesssionPersistence methods for reference
  ...
  async sortedTodos(todoList) {
    ...
    // NAIEVE SOLUTION
    // const SORTED_DONE = "SELECT * FROM todos WHERE todolist_id = $1 AND done = true ORDER BY title ASC";
    // const SORTED_UNDONE = "SELECT * FROM todos WHERE todolist_id = $1 AND done = false ORDER BY title ASC";

    // let sortedDone = dbQuery(SORTED_DONE, todoList.id);
    // let sortedUndone = dbQuery(SORTED_UNDONE, todoList.id);
    // let allSorted = await Promise.all([sortedUndone, sortedDone]);

    // let todos = allSorted[0].rows.concat(allSorted[1].rows);
    // console.log(todos);
    // if (!todos) return undefined;
